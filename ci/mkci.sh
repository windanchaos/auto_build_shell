...
##############################################################################
### 1      若脚本不存在上传restart脚本 接收参数: 远程配置数组索引$1
##############################################################################
...
function init_checkout_code(){
    ...
    if [[ ! -d $1/${project_name} ]]; then
        echo "工程文件夹不存在，开始从资源库 [ ${repository_url} ] 检出代码到 [ $1 ]" 
        case "${checkout_command[0]}" in
            svn )
        ...
        echo "工程文件夹已存在但内容为空，开始从资源库 [ ${repository_url} ] 检出代码到 [ $1 ]" 
        case "${checkout_command[0]}" in
            svn )
                cd $1 && $2 ${repository_url} --username ${svn_username} --password ${svn_password}
                ;;
...
show_banner
##############################################################################
### 1   变量声明 var=value 等号必须前后紧挨着
##############################################################################
if   [[ ! -n ${MKCI_HOME} ]]; 
then 
    echo "[info] "
    echo "[info] 环境变量MKCI_HOME未设置，请设置后再执行"
    exit 0
else 
    echo "[info] "
    echo "[info] MKCI_HOME INFO"
    echo "[info] ------------------------------------------------------------------------"
    echo "[info] MKCI_HOME : ${MKCI_HOME}"
    echo "[info] ------------------------------------------------------------------------"
fi
#配置文件路径名称
# conf_filename=~/conf.ini
conf_filename=${MKCI_HOME}/conf/conf.ini
